...
NEIGHBOR_THRESHOLD = 133  # Threshold for cell death based on neighbors
...
drawing = False
paused = False
allow_pause_while_drawing = True
...
def apply_rules(grid, larger_neighborhood_radius=1):
    neighbors = (
    ...
    # Add a larger neighborhood if radius is greater than 0
    if larger_neighborhood_radius > 0:
        larger_neighborhood = np.zeros_like(grid)
        for i in range(1, larger_neighborhood_radius + 1):
            larger_neighborhood += (
                np.roll(grid, (i, 0), (0, 1)) +
                np.roll(grid, (-i, 0), (0, 1)) +
                np.roll(grid, (0, i), (0, 1)) +
        ...
        new_grid = (grid + neighbors + neighbors_rotated + larger_neighborhood) % MODULUS
    else:
