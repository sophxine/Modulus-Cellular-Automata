#Some rules are very flashy so proceed with caution if that would affect you.
...
CONSERVATISM = 0.26  # Conservatism factor (adjust as needed, 1 = disabled)
NEIGHBOR_THRESHOLD = 512  # Threshold for cell death based on neighbors
...
# Define a fixed pattern for conservativism
conservativism_pattern = np.random.rand(WIDTH // CELL_SIZE, HEIGHT // CELL_SIZE) < CONSERVATISM

# Keep track of the current drawing position
current_draw_position = None

# Define the transition rules with translational and rotational symmetry, and cell death
def apply_rules(grid):
    # Create larger neighborhoods
    larger_neighbors = (
        ...
        np.roll(grid, (2, 0), (0, 1)) +
        np.roll(grid, (-2, 0), (0, 1)) +
        np.roll(grid, (0, 2), (0, 1)) +
        np.roll(grid, (0, -2), (0, 1))
    )
    ...
    new_grid = (grid + larger_neighbors + larger_neighbors_rotated) % MODULUS

    # Apply cell death rule based on neighborhood count
    death_mask = (larger_neighbors >= NEIGHBOR_THRESHOLD)
    new_grid = np.where(death_mask, 0, new_grid)

    # Apply conservativism using the fixed pattern
    new_grid = np.where(conservativism_pattern, new_grid, grid)
